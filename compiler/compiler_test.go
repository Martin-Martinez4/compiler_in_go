...
func testInstructions(expected []code.Instructions, actual code.Instructions) error {
	...
	for i, ins := range concatted {
		if actual[i] != ins {
			fmt.Errorf("wrong instruction as %d\nwant=%q\ngot=%q", i, concatted, actual)
		}
	}
...
func testIntergerObject(expected int64, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object was wrong value.  got=%d, want=%d", result.Value, expected)
	}
...
func testConstants(t *testing.T, expected []interface{}, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case int:
			err := testIntergerObject(int64(constant), actual[i])
			if err != nil {
				return fmt.Errorf("consant %d - testIntegerObject failed: %s", i, err)
			}
	...
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedConstants: []interface{}{
				...
				[]code.Instructions{
					code.Make(code.OpConstant, 0),
					code.Make(code.OpPop), code.Make(code.OpConstant, 1),
					code.Make(code.OpReturnValue),
				},
			},
			expectedInstructions: []code.Instructions{
