...
func testIntergetObject(expected int64, actual object.Object) error {
...
func testStringObject(expected string, actual object.Object) error {
	...
	if !ok {
		return fmt.Errorf("object is not a Srting. got=%T (%+v)", actual, actual)
	}
...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case int:
		err := testIntergetObject(int64(expected), actual)
		if err != nil {
	...
	case *object.Error:
		...
		if !ok {
			t.Errorf("object is not an Error: %T (%+v)", actual, actual)
			return
		}
	...
	case []int:
		...
		for i, expectedElem := range expected {
			err := testIntergetObject(int64(expectedElem), array.Elements[i])
			if err != nil {
				t.Errorf("testIntegerObject failed: %s", err)
			}
	...
	case map[object.HashKey]int64:
		...
		for expectedKey, expectedValue := range expected {
			...
			err := testIntergetObject(expectedValue, pair.Value)
			if err != nil {
				t.Errorf("testIntegerObject failed: %s", err)
			}
		}
...
func TestCallingFunctionsWithoutArguments(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `
				let one = fn() { 1; }; let two = fn() { 2; };
				one() + two() `,
			expected: 3,
		},
		{
			input: `
				let a = fn() { 1 }; let b = fn() { a() + 1 }; let c = fn() { b() + 1 };
				c(); `,
			expected: 3,
		},
	}
...
func TestFunctionsWithoutReturnValue(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `
				let noReturn = fn() { }; let noReturnTwo = fn() { noReturn(); }; noReturn();
				noReturnTwo(); `,
			expected: Null,
		},
	}
...
func TestFirstClassFunctions(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
				let returnsOne = fn() { 1; }; let returnsOneReturner = fn() { returnsOne; };
				returnsOneReturner()(); `,
			expected: 1,
		},
		{
...
func TestClosures(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `
				let a = 1; let newAdderOuter = fn(b) { fn(c) {
				fn(d) { a + b + c + d }; }; };
...
func TestRecursiveFunctions(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `
				let wrapper = fn() { let countDown = fn(x) { if (x == 0) { return 0;
				} else { countDown(x - 1); } }; countDown(1); };
				wrapper(); `,
			expected: 0,
		},
	}
